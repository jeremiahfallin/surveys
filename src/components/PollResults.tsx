  ...
  const renderResults = () => {
    switch (poll.votingFormat) {
      ...
      case "plurality": {
        ...
        const totalVoters = poll.pluralityVotes.length;
        ...
        return (
          <Flex direction="column" gap="3">
            {voteCounts.map((option, index) => {
              const percentage =
                totalVoters > 0 ? (option.count / totalVoters) * 100 : 0;
              return (
                <Box key={index}>
                  <Flex justify="between" mb="2">
                    <Text>{option.text}</Text>
            ...
            {ratings.map((option, index) => {
              const winRate =
                option.comparisons > 0
                  ? (option.wins / option.comparisons) * 100
                  : 0;
              return (
                <Box key={index}>
                  <Flex justify="between" mb="2">
